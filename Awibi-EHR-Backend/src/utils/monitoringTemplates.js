...
const MONITORING_TEMPLATES = [
  ...
  {
    type: 'ELECTROLYTE_CORRECTION',
    label: 'Electrolyte correction infusion',
    targetUnit: 'mmol/L',
    frequencyMins: 60,
    tracksIntake: true,
    fields: [
      { key: 'electrolyte', label: 'Electrolyte', kind: 'select', required: true,
        options: ['Potassium', 'Sodium', 'Calcium', 'Magnesium', 'Phosphate'] },
      { key: 'baselineValue', label: 'Baseline result', unit: 'mmol/L', kind: 'number' },
      { key: 'replacement', label: 'Replacement prescribed', kind: 'text', required: true },
      { key: 'volumeMl', label: 'Volume infused', unit: 'ml', kind: 'number', mapsTo: 'intakeMl' },
      { key: 'rateMlHr', label: 'Infusion rate', unit: 'ml/hr', kind: 'number' },
      { key: 'latestValue', label: 'Repeat electrolyte', unit: 'mmol/L', kind: 'number' },
      { key: 'ecgObserved', label: 'ECG monitoring in place', kind: 'boolean' },
      { key: 'symptoms', label: 'Symptoms or IV-site concern', kind: 'text' },
    ],
  },
  {
  ...
  {
    type: 'PRESSURE_AREA_REPOSITIONING',
    label: 'Pressure-area repositioning',
    frequencyMins: 120,
    fields: [
      { key: 'position', label: 'New position', kind: 'select', required: true,
        options: ['Left lateral', 'Right lateral', 'Supine', 'Prone', '30° tilt', 'Chair', 'Other'] },
      { key: 'repositioned', label: 'Position changed', kind: 'boolean', required: true },
      { key: 'pressureArea', label: 'Pressure area checked', kind: 'text', required: true },
      { key: 'skinCondition', label: 'Skin condition', kind: 'select', required: true,
        options: ['Intact', 'Blanching redness', 'Non-blanching redness', 'Blister / skin break', 'Moisture lesion', 'Dressing present'],
        normalOptions: ['Intact', 'Blanching redness'] },
      { key: 'supportSurface', label: 'Support / pressure relief used', kind: 'text' },
      { key: 'assistance', label: 'Assistance required', kind: 'select', options: ['Independent', 'One person', 'Two people', 'Hoist / device'] },
      { key: 'painScore', label: 'Pain score (0-10)', kind: 'number', goalMin: 0, goalMax: 3, criticalHigh: 8 },
      { key: 'concern', label: 'Skin or comfort concern', kind: 'text' },
    ],
  },
  {
